...
import { ArrowRight, LayoutGrid, Calendar } from "lucide-react"
import { BlogCard } from "@/components/blog/BlogCard"
import { CalendarView } from "@/components/blog/CalendarView"
import { getCommunityPostStatsBySlugs, getCommunityPosts } from "@/lib/supabase/queries"
import { isPerformanceEndedKst } from "@/lib/date"

type SearchParams = Promise<{ tag?: string; view?: string }>

export const metadata: Metadata = {
  title: "공연 소개 — 알터즈",
  description:
    "알터즈가 관객 언어로 풀어 소개하는 공연들. SNS에서 소개한 공연부터 지금 주목할 작품까지 만나보세요.",
  alternates: { canonical: "/blog" },
  openGraph: {
    title: "공연 소개 — 알터즈",
    description: "알터즈가 풀어 소개하는 공연 이야기",
    url: "/blog",
    type: "website",
  },
}
...
export default async function BlogPage({ searchParams }: { searchParams: SearchParams }) {
  const { tag: activeTag, view: activeView } = await searchParams
  const isCalendarView = activeView === "calendar"
  const [posts, manualStats] = await Promise.all([
    getCommunityPosts(),
  ...
  const displayPosts = sortPostsByPopularity(
    posts.length > 0
      ? [
          ...manualPosts.filter(
            (manualPost) => !posts.some((post) => post.slug === manualPost.slug),
          ),
          ...posts,
        ]
      : manualPosts,
  )
  const isExample = posts.length === 0
  ...
  const filteredPosts = activeTag
    ? displayPosts.filter((p) => p.tags?.includes(activeTag))
    : displayPosts

  const ongoingPosts = filteredPosts.filter((p) => !isPerformanceEnded(p.performance_end_date))
  const archivedPosts = filteredPosts.filter((p) => isPerformanceEnded(p.performance_end_date))
  const activePosts = ongoingPosts.length > 0 ? ongoingPosts : filteredPosts
    ...
    <div className="mx-auto max-w-6xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
        ...
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-3">
            ...
            <p className="max-w-xl text-sm text-muted-foreground sm:text-base">
              ...
              알터즈가 관객 언어로 풀어 전하는 공연 이야기입니다.
            </p>
          </div>
          {/* 뷰 토글 */}
          <div className="flex shrink-0 items-center gap-1 border border-border p-0.5">
            <Link
              ...
              className={`flex items-center gap-1.5 px-3 py-2 text-xs font-semibold transition ${
                !isCalendarView ? "bg-foreground text-background" : "text-muted-foreground hover:text-foreground"
              }`}
            >
            ...
            <Link
              href={activeTag ? `/blog?tag=${encodeURIComponent(activeTag)}&view=calendar` : "/blog?view=calendar"}
              className={`flex items-center gap-1.5 px-3 py-2 text-xs font-semibold transition ${
                isCalendarView ? "bg-foreground text-background" : "text-muted-foreground hover:text-foreground"
              }`}
            >
      ...
      {allTags.length > 0 && (
        <div className="mb-8 flex flex-wrap gap-2">
          <Link
            href="/blog"
            className={`px-3 py-1.5 text-xs font-semibold transition border ${
              !activeTag
      ...
      {/* 캘린더 뷰 */}
      {isCalendarView && (
        <CalendarView posts={displayPosts} />
      )}

      {/* 예시 안내 배너 (실제 포스트 없을 때만, 목록 뷰에서만) */}
      {!isCalendarView && isExample && (
        <div className="mb-8 border-l-4 border-primary bg-primary/5 px-4 py-3">
          <p className="text-sm font-semibold text-primary">✦ 에디터 예시 글</p>
          <p className="mt-0.5 text-xs text-muted-foreground">
            아직 정식 소개글이 준비되지 않았어요. 아래는 알터즈 공연 소개의 예시입니다.
          </p>
        </div>
      )}
      ...
      {!isCalendarView && (
        <>
          ...
          {activeTag && filteredPosts.length === 0 && (
            <div className="py-16 text-center">
              <p className="text-sm text-muted-foreground">
                <span className="font-semibold text-foreground">#{activeTag}</span> 태그의 공연 소개가 없습니다.
              </p>
              <Link href="/blog" className="mt-4 inline-block text-xs font-semibold text-primary underline-offset-2 hover:underline">
                전체 목록 보기
              </Link>
            </div>
          )}
          ...
          {/* 더보기 */}
          {!isExample && posts.length >= 6 && (
            <div className="mt-10 flex justify-center">
              <Link
                href="/blog?page=2"
                className="inline-flex items-center gap-2 border border-border px-6 py-2.5 text-sm font-semibold text-foreground/70 transition hover:border-primary/40 hover:text-primary"
              >
                더 많은 공연 소개 보기
